...
if not api_key:
    ...
    st.info("앱을 실행하기 전에 환경 변수를 설정하거나 코드에 API 키를 직접 입력하세요.")
    st.stop()
else:
    try:
        client = Together(api_key=api_key)
    except Exception as e:
        st.error(f"API 클라이언트 초기화에 실패했습니다: {e}")
        st.stop()
...
def retrieve_relevant_laws(query_text, n_results=5):
    collection = get_rag_retriever()
    if not collection:
        return ""
    try:
        ...
        if results['documents']:
            for i, (doc, meta) in enumerate(zip(results['documents'][0], results['metadatas'][0])):
                ref_id = meta.get('id', f"관련법령_{i+1}")
                context_str += f"- [{ref_id}]: {doc}\n"
        return context_str
    except Exception:
        return ""
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": st.session_state.contract_text}
                ],
                temperature=0.1,
